...
function liquid_get_header_view()
{
	...
	if (
		...
		'ld-product-sizeguide' === get_post_type() ||
		'liquid-sticky-atc' === get_post_type()
	) {
		return;
	}
	...
	if (is_search()) {
	...
	} elseif ( liquid_helper()->get_current_page_id() === get_option('page_for_posts' ) ) {
		...
		$enable_titlebar = !empty( $enable_titlebar ) ? $enable_titlebar : liquid_helper()->get_option('blog-title-bar-enable', 'raw', '');
