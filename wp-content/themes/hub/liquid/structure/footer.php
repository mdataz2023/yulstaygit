...
function liquid_attributes_footer( $attributes ) {
	...
	if ( class_exists( 'Liquid_Elementor_Addons' ) && defined('ELEMENTOR_VERSION') ){
		...
		if ( $footer_fixed === 'on' && $page_settings_model->get_settings( 'footer_body_clip' ) === 'yes' ){
			wp_enqueue_script(
				'liquid-body-clip',
				get_template_directory_uri() . '/assets/js/body-clip/liquidBodyClip.min.js',
				['liquid-theme'],
				null
			);
		}
...
function liquid_footer_backtotop() {
	...
	$atts = array(
		...
		'href'     => '#wrap',
		'nofollow' => true,
		'text'     => esc_html__( 'Return to top of page', 'hub' ),
	);
...
function liquid_get_custom_footer_id() {
	...
	if ( liquid_helper()->get_option('footer-force') === 'on' ){
		if ( !empty( $id = liquid_helper()->get_option( 'footer-template' ) ) ){
			return $id;
		}
	}
