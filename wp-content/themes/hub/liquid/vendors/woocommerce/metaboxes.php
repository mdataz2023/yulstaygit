...
function liquid_add_custom_general_fields() {
  ...
  echo '<div class="options_group">';
	...
	woocommerce_wp_textarea_input(
		array(
			'id'          => '_custom_label',
			'label'       => esc_html__( 'Custom label', 'hub' ),
			'desc_tip'    => 'true',
			'description' => esc_html__( 'Input a custom label for the product', 'hub' )
		)
	);
	echo '</div>';
...
?>
	<tr class="form-field">
		<th scope="row">
			<label for="term-background-color"><?php esc_html_e( 'Background Color', 'hub' ); ?></label>
			<td>
				...
				<p class="description"><?php esc_html_e( 'Select a background for product category label', 'hub' ); ?></p>
			</td>
		</th>
	</tr>
<?php
...
function liquid_create_background_meta_field() {
	...
	<div class="form-field term-background-wrap">
		...
		<p><?php esc_html_e( 'Select a background for product category label', 'hub' ); ?></p>
	</div>
<?php }