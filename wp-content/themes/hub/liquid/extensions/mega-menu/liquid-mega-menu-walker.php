	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( $item->thumbnail_id ) {
			...
			if( 'left' === $item->liquid_icon_position ) {
				...
				$args->link_before = '<span class="link-icon d-inline-flex hide-if-empty left-icon">' . $custom_icon . '</span>' . $args->link_before;
			} 
			elseif( 'center' === $item->liquid_icon_position ) {
				$item->classes[] = 'center-icon';
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon d-inline-flex hide-if-empty center-icon">' . $custom_icon . '</span>' . $args->link_before;
			}
			else {
				$args->old_link_after = $args->link_after;
				$args->link_after = $args->link_after . '<span class="link-icon d-inline-flex hide-if-empty right-icon">' . $custom_icon . '</span>';			
			}
		}
		...
		if( ! empty( $item->liquid_icon ) ) {
			if( 'left' === $item->liquid_icon_position ) {
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon d-inline-flex hide-if-empty left-icon"><i class="'. esc_attr( $item->liquid_icon ) .'"></i></span>' . $args->link_before;
			} 
			elseif( 'center' === $item->liquid_icon_position ) {
				$item->classes[] = 'center-icon';
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon d-inline-flex hide-if-empty center-icon"><i class="'. esc_attr( $item->liquid_icon ) .'"></i></span>' . $args->link_before;
			}
			else {
				$args->old_link_after = $args->link_after;
				$args->link_after = $args->link_after . '<span class="link-icon d-inline-flex hide-if-empty right-icon"><i class="'. esc_attr( $item->liquid_icon ) .'"></i></span>';				
			}
		}
	...
	function get_megamenu( $id ) {
		...
		if( !empty( $id ) ) {
			if ( class_exists( 'Liquid_Elementor_Addons' ) && defined('ELEMENTOR_VERSION') ){
				...
				$globals = $page_settings_model->get_settings('__globals__');

				if ( (isset($globals) && !empty($globals)) && (isset($globals['megamenu_bg_color']) && !empty($globals['megamenu_bg_color'])) ) {
					$color = 'var(--e-global-color-' . explode( '=', $globals['megamenu_bg_color'] )[1] . ')';
				}
				...
				if ( $bg_type === 'classic' && ! empty( $color ) ) {
					...
					if ( (isset($globals) && !empty($globals)) && (isset($globals['megamenu_bg_color_b']) && !empty($globals['megamenu_bg_color_b'])) ) {
						$gradient_color_b = 'var(--e-global-color-' . explode( '=', $globals['megamenu_bg_color_b'] )[1] . ')';
					}
