...
class Liquid_Mega_Menu_Manager extends Liquid_Base {
    ...
    function setup_nav_menu_item( $menu_item ) {
		...
		$menu_item->liquid_color = get_post_meta( $menu_item->ID, '_menu_item_liquid_color', true );
	...
	function update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_data ) {
		...
		if ( isset( $_REQUEST['menu-item-liquid-color'][$menu_item_db_id] ) ) {
			update_post_meta($menu_item_db_id, '_menu_item_liquid_color', $_REQUEST['menu-item-liquid-color'][$menu_item_db_id]);
		}
