...
// Liquid Extra CPT support filters
if ( isset( get_option('liquid_one_opt')['liquid_extra_cpt_support'] ) && !empty( get_option('liquid_one_opt')['liquid_extra_cpt_support'] ) ) {

	add_filter( 'liquid_hfp_cpt' , function( $cpt ){ 

		$liquid_extra_cpt_support = explode( "\n", str_replace("\r", "", get_option('liquid_one_opt')['liquid_extra_cpt_support'] ) );
		$cpt = array_unique(array_merge($cpt, $liquid_extra_cpt_support));

		return $cpt; 

	} );

	add_filter( 'liquid_sidebar_cpt' , function( $cpt ){ 
	...
	add_filter( 'liquid_titlewrapper_cpt' , function( $cpt ){ 
